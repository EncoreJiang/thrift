...
public:
  ...
  */
  ...
  void generate_struct(t_struct* tstruct);
    ...
    std::string get_real_go_module(const t_program* program) {
      if (program == program_)
      {
      ...
      if (!package_flag.empty()) {
        std::string module = package_flag;
        size_t dot = module.rfind('.');
        if (dot != string::npos) {
            module = module.substr(0, dot);
        }
        return module + "." + lowercase(program->get_name());
      }
        return lowercase(program->get_name());
    }
private:
...
THRIFT_REGISTER_GENERATOR(json, "JSON", "    package=  Package name (default: inferred from thrift file name)")
